...
def seed_data():
    ...
    # Clear existing data
    profile_picture_upload_folder = os.getenv('PROFILE_PICTURE_UPLOAD_FOLDER')
    for filename in os.listdir(profile_picture_upload_folder):
        if filename != 'default.jpg':
            file_path = os.path.join(profile_picture_upload_folder, filename)
            if os.path.isfile(file_path):
                os.remove(file_path)

    question_image_upload_folder = os.getenv('QUESTION_IMAGE_UPLOAD_FOLDER')
    for filename in os.listdir(question_image_upload_folder):
        file_path = os.path.join(question_image_upload_folder, filename)
        if os.path.isfile(file_path):
            os.remove(file_path)
    db.drop_all()
    db.create_all()
    ...
    # Login and take quizzes through API
    import requests
    ...
    # Take quizzes for first 10 students
    for student in students[:10]:
        # Login as student
        token = get_auth_token(student.user.email, "12345678")
        headers = {"Authorization": f"Bearer {token}"}

        # Each student takes first 5 quizzes
        for quiz in quizzes[:5]:
            try:
                # Start the quiz
                start_response = requests.post(
                    f"{base_url}/quizzes/{quiz.id}/take_quiz",
                    headers=headers
                )
                if start_response.status_code != 200:
                    continue

                take_quiz_data = start_response.json()["data"]
                take_quiz_id = take_quiz_data["take_quiz_id"]

                # Generate mock quiz answers
                quiz_state = {}
                for idx, question in enumerate(quiz.chapter.questions):
                    if question.question_type.value == "MCQ":
                        # Select first option for MCQ
                        correct_option = question.options[0].id
                        quiz_state[str(question.id)] = {"selected_values": [correct_option]}
                    elif question.question_type.value == "MSQ":
                        # Select first two options for MSQ
                        correct_options = [opt.id for opt in question.options[:2]]
                        quiz_state[str(question.id)] = {"selected_values": correct_options}
                    elif question.question_type.value == "NUMERICAL":
                        # Use the correct answer for numerical
                        answer = question.correct_answers[0].answer_text
                        quiz_state[str(question.id)] = {"selected_values": [answer]}

                # Submit the quiz
                submit_response = requests.post(
                    f"{base_url}/take_quiz/{take_quiz_id}/submit",
                    headers=headers,
                    json={"quiz_state": quiz_state}
                )
                
                if submit_response.status_code == 200:
                    print(f"Quiz {quiz.id} completed for student {student.user.email}")

            except Exception as e:
                print(f"Error taking quiz {quiz.id} for student {student.user.email}: {str(e)}")

    db.session.commit()
    print("Database seeded with enhanced sample data!")
