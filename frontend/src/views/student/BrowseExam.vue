                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-4">
                            <div class="input-group">
                                ...
                                <input type="text"
                                    class="form-control form-control-sm" placeholder="Search exams..."
                        ...
                        <div class="mb-4">
                            ...
                            <div class="d-flex align-items-center">
                                ...
                                <select class="form-select form-select-sm me-2"
                                    ...
                                    <option value="date">Date of Quiz</option>
            ...
            <div class="col-md-9 pe-0">
                ...
                <div class="row">
                    ...
                    <template v-else-if="filteredExams.length">
                        <div v-for="quiz in filteredExams" :key="quiz.id" class="col-md-6 mb-4">
                            <div class="card" :style="{
                                opacity: quiz.status === 'expired' ? 0.5 : 1,
                                pointerEvents: quiz.status === 'expired' ? 'none' : 'auto'
                            }">
                                ...
                                <div class="card-footer border-top p-0 rounded-bottom-4">
                                    ...
                                    <button v-if="quiz.attempted && quiz.completed" class="btn btn-primary w-100 rounded-top-0"
                                        disabled>
                                        Already Completed
                                    </button>
                                    <button @click="router.push({name: 'QuizInterface', params:{id: quiz.id}})" v-else-if="quiz.attempted && !quiz.completed && !(quiz.status == 'expired')" class="btn btn-success w-100 rounded-top-0">
                                        Continue Exam
                                    </button>
                                    <button v-else-if="quiz.status == 'upcoming'" class="btn btn-primary w-100 rounded-top-0"
                                        disabled>
                                        Starting on {{ useDateFormatter().formatDate2(quiz.date_of_quiz) }}
                                    </button>
                                    <button v-else-if="quiz.status == 'live'"
                                        class="btn btn-primary w-100 rounded-top-0" @click="takeExam(quiz.id)">
                                        Take Exam
                                    </button>
                                    <button v-else-if="quiz.status == 'expired'" disabled
                                        class="btn btn-secondary w-100 rounded-top-0">
                                        Exam Expired
                                    </button>
                                </div>
                    ...
                    <div v-else class="col-md-12 text-center">
                        <p class="text-muted fs-5">No exams found matching your criteria.</p>
                    </div>
                </div>
            </div>
...
const statuses = ref(['upcoming', 'live', 'expired'])
...
const fetchExams = async () => {
    try {
        loading.value = true
        const response = await axios.get('/api/quizzes/exams')
        exams.value = response.data.data
        subjects.value = [...new Set(exams.value.map(quiz => quiz.subject_name))]
        loading.value = false
    } catch (error) {
        loading.value = false
...
const takeExam = (quizId) => {
    router.push({ name: 'QuizInterface', params: { id: quizId } })
}
