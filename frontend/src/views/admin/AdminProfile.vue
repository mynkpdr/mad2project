            ...
            <div class="col-lg-12">
                <div class="row m-4">
                    <div class="col-md-4 mb-4">
                        <div class="card">
                            <div class="card-body text-center">
                                ...
                                <i v-if="previewImage" class="bi bi-arrow-right-circle-fill fs-4 fw-bold"></i>
                                <img v-if="previewImage" :src="previewImage" alt="preview"
                                    class="rounded-circle img-fluid mb-3"
                                    style="width: 150px; height: 150px; object-fit: cover;">
                                <div v-if="isEditing" class="mb-3">
                                    <label for="profile_picture" class="form-label">Change Profile Picture</label>
                                    <input type="file" id="profilePicture" ref="fileInput" accept="image/*"
                                        @change="onFileSelected" class="form-control">
                    ...
                    <div class="col-md-8">
                            ...
                            <div class="card-body">
                                <form @submit.prevent="saveChanges" v-if="isEditing">
                                    ...
                                    <div class="mb-3 row">
                                        ...
                                        <div class="col-sm-9">
                                            <input disabled type="email" class="form-control bg-muted"
                                                v-model="user.email" required style="opacity: 0.5;">
                                        </div>
                                    </div>
                                    <div class="mb-3 row">
                                    ...
                                    <div class="mb-3 row">
                                        ...
                                        <div class="col-sm-9">
                                            <textarea type="text" class="form-control" v-model="user.address">
                                    </textarea>
                                        </div>
                                    </div>
                                    <div class="mb-3 row">
                                ...
                                <div v-else>
                                    ...
                                    <p><strong>Status:</strong> <span
                                            :class="['badge', user.status.toUpperCase() === 'ACTIVE' ? 'bg-success' : 'bg-warning']">{{
                                                user.status }}</span></p>
                                    <p><strong>Member Since:</strong> {{
                                        useDateFormatter().formatDate2(user.date_created)
                                        }}
                                    </p>
                                    <p><strong>Last Login:</strong> {{ useDateFormatter().formatTimeAgo(user.last_login)
                                        }}
                                    </p>
                                </div>
                            </div>
                        ...
                        <div class="card mt-4">
                            <div class="card-header">
                                <h4 class="card-title mb-0">Change Password <span class="float-end">
                                        <router-link class="btn text-danger" :to="{ name: 'ForgotPassword' }">Forgot
                            ...
                            <div class="card-body">
                                <form @submit.prevent="resetPassword">
                                    <div class="mb-3">
                                        ...
                                        <div class="input-group">
                                            <input :type="showCurrentPassword ? 'text' : 'password'"
                                                ...
                                                v-model="passwordReset.current_password" required minlength="8">
                                            <span class="input-group-text" @click="toggleCurrentPassword"
                                                style="cursor: pointer;">
                                                <i
                                                    :class="showCurrentPassword ? 'bi bi-eye-fill' : 'bi bi-eye-slash-fill'"></i>
                                            </span>
                                        </div>
                                    </div>
                                    <div class="mb-3">
                                        ...
                                        <div class="input-group">
                                            <input :type="showPassword ? 'text' : 'password'" class="form-control"
                                                id="new_password" v-model="passwordReset.new_password" required
                                                minlength="8">
                                            <span class="input-group-text" @click="togglePassword"
                                                style="cursor: pointer;">
                                                <i
                                                    :class="showPassword ? 'bi bi-eye-fill' : 'bi bi-eye-slash-fill'"></i>
                                            </span>
                                        </div>
                                    </div>
                                    <div class="mb-3">
                                        ...
                                        <div class="input-group">
                                            <input :type="showPassword ? 'text' : 'password'" class="form-control"
                                                id="confirm_password" v-model="passwordReset.confirm_password" required
                                                minlength="8">
                                            <span class="input-group-text" @click="togglePassword"
                                                style="cursor: pointer;">
                                                <i
                                    ...
                                    <p v-if="passwordReset.confirm_password && !(passwordReset.confirm_password == passwordReset.new_password)"
                                        class="text-danger">Both passwords should match</p>
                                    <button type="submit" class="btn btn-success"
                                        :disabled="!(passwordReset.current_password && passwordReset.new_password && passwordReset.confirm_password && passwordReset.confirm_password == passwordReset.new_password)">Reset
                                        Password</button>
                                </form>
                            </div>
                        </div>
...
const uploadFile = async () => {
    ...
    try {
        ...
        const response2 = await axios.put('/api/admins/profile', {
...
const getProfile = async () => {
    try {
        ...
        user.dob = response.data.dob ? new Date(response.data.dob * 1000).toISOString().split('T')[0] : null
        user.profile_picture = response.data.profile_picture
        user.email_verified = response.data.email_verified
        user.status = response.data.status
        user.date_created = response.data.date_created
        user.last_login = response.data.last_login
        user.qualification = response.data.qualification
...
const resetPassword = async () => {
    try {
        await axios.post("/api/auth/reset-password", {
            current_password: passwordReset.current_password,
            new_password: passwordReset.new_password,
        })
        push.success("Your password was successfully reset.")
